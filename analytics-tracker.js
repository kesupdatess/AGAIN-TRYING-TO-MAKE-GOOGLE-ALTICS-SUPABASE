...
// Project: Events Database (uegdizdnminwhkxcdcle)
// ═══════════════════════════════════════════════════════
...
document.addEventListener('click', async (e) => {
  ...
  if (notice) {
    ...
    await trackEvent('notice_click', 'Notices', title, null, { notice_url: notice.href });
  }
});
...
document.addEventListener('click', async (e) => {
  ...
  if (link) {
    await trackEvent('erp_click', 'ERP', link.innerText.trim(), null, { link_url: link.href });
  }
});
...
document.addEventListener('click', async (e) => {
  ...
  if (link) {
    await trackEvent('library_click', 'Library', link.innerText.trim(), null, { link_url: link.href });
  }
});
...
document.addEventListener('input', async (e) => {
  if (e.target.id === 'searchInput') {
    ...
    if (query.length > 2) {
      await trackEvent('search', 'Search', query, null, { query_length: query.length });
    }
  }
});
